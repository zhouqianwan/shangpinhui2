          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="attr in spuSaleAttrList" :key='attr.id'>
                ...
                <dd changepirce="0" :class="{active:attrValue.isChecked == 1}" v-for="attrValue in attr.spuSaleAttrValueList" :key="attrValue.id" @click="changeIsChecked(attrValue,attr.spuSaleAttrValueList)">{{attrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
              ...
              <div class="add">
                <a @click="goAddCartSuccess">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    // 点击属性，经属性进行切换
    changeIsChecked(attrValue, arr) {
      // 让这个数组中全部的isChecked都清除，让他的isChecked为1
      arr.forEach(item => {
        item.isChecked = 0
      })
    ...
    async goAddCartSuccess() {
      try {
        await this.$store.dispatch('addToShopcart', { skuId: this.skuInfo.id, skuNum: this.num })
      } catch (error) {
        alert(error.message)
      }
      // 会话存储，将对象类型的商品信息通过sessionStorage存储起来
            ...
            dl {
              ...
              dd {
                ...
                &.active {
                  ...
                  background-color: rgb(178, 221, 239);
                }
              }
            }
          }
