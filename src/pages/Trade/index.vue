<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(item,index) in addres" :key="index">
        <span class="username " :class="{selected:item.active==1}">{{item.userName}}</span>
        <!-- 点击那一个地址信息，就把当前地址信息选为默认地址 -->
        <p @click="changeSelect(item)">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        ...
        收货人：<span>{{defaultAddress.userName}} </span>
        <span>{{defaultAddress.phone}}</span>
      </div>
    </div>
    <div class="sub clearFix">
      <a class="subBtn" @click.prevent="commitOrder">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    // 获取addres数据
    ...mapState({
      addres: state => state.trade.addres,
      orderInfo: state => state.trade.orderInfo
    }),
    // 通过计算属性，拿到当前active==1的地址
    defaultAddress() {
      return (
        this.addres.find(item => {
  ...
  methods: {
    ...
    async commitOrder() {
      let data = {
        // 收件人姓名
        consignee: this.defaultAddress.userName,
        // 收件人电话
      ...
      try {
        await this.$store.dispatch('commitOrder', { tradeNo, data })
        let orderId = this.$store.state.trade.orderId
